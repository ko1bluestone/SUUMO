...
# テーブルの作成
table_name = "RealEstateTable4"
columns = "id INT AUTO_INCREMENT PRIMARY KEY, 物件名 VARCHAR(255), カテゴリ VARCHAR(255), 住所 VARCHAR(255), 築年数 FLOAT, ビル高さ FLOAT, 階数 FLOAT, 家賃 FLOAT, 間取り VARCHAR(255), 面積 FLOAT, URL VARCHAR(255), 都道府県 VARCHAR(255), 市区町村 VARCHAR(255), その他住所 VARCHAR(255)"
...
def insert_data(connection, table_name, data):
    ...
    try:
        for index, row in data.iterrows():
            insert_query = f"INSERT INTO {table_name} (物件名, カテゴリ, 住所, 築年数, ビル高さ,階数, 家賃,間取り,面積,URL,都道府県,市区町村 ,その他住所) " \
                           f"VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s);"
            cursor.execute(insert_query, (row['物件名'], row['カテゴリ'], row['住所'], row['築年数'], row['ビル高さ'],
                                          row['階数'], row['家賃'], row['間取り'], row['面積'], row['URL'], row['都道府県'], row['市区町村'], row['その他住所']))
        connection.commit()
        ...
        # データセットの挿入
insert_data(connection, "RealEstateTable4", df)
